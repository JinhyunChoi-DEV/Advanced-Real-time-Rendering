...
//	glUniform3fv(loc, 1, &tr[0]);
	...
	shadowFBO.BindTexture(5, programId, "shadowMap");
...
void SampleScene::CreateShader()
{
	...
	int loc = glGetUniformBlockIndex(computeVerticalShader->programId, "blurKernel");
	glUniformBlockBinding(computeVerticalShader->programId, loc, 0);
	...
	loc = glGetUniformBlockIndex(computeHorizontalShader->programId, "blurKernel");
	glUniformBlockBinding(computeHorizontalShader->programId, loc, 1);
